...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Each transaction includes its type (payment/refund/void/retry_N/Failed/etc.), order_status, "
            "and payout_status which is one of: "
            ...
            "'payment_failed' (the buyer's subscription charge failed and is retrying — type will be "
            "retry_1/retry_2/retry_3/retrycancel_4/Failed; date is the next scheduled retry attempt), "
...
def _taskbot_analyze(message: str, context: str, sender_name: str, inbox_name: str) -> dict:
    ...
    prompt = (
        ...
        "Flag as action required ONLY when the message contains:\n"
        "- A complaint, dispute, chargeback, or refund request\n"
        "- An explicit request to speak with a human/manager/supervisor\n"
        "- A cancellation or account-closure request\n"
        "- A legal threat, BBB mention, or regulatory complaint\n"
        "- A time-sensitive emergency (e.g., fraud, unauthorized charge)\n"
        "- An HR/admin/internal-business request\n"
        "- A purchase/sale offer for tradelines from a seller or bulk buyer\n"
        "- Something unusual or complex the auto-responder clearly cannot handle\n\n"
        "Do NOT flag:\n"
        "- Routine questions about tradelines, pricing, process, or timing\n"
        "- Casual inquiries or exploratory questions (e.g. 'wondering about a tradeline')\n"
        "- Simple greetings or 'thank you' messages\n"
        "- Messages already being handled in the conversation\n"
        "- Requests for information the bot can answer from the knowledgebase\n"
        "- Automated/system messages or spam\n\n"
        "When in doubt, do NOT flag. The draft bot will handle routine inquiries.\n\n"
        f"Inbox: {inbox_name}\n"
